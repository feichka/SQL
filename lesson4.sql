...
FROM likes
WHERE user_id = (SELECT user_id FROM `profiles` WHERE user_id = 
	(SELECT id FROM users WHERE id = likes.user_id)
    AND birthday > '2011-04-23');

-- Определить кто больше поставил лайков (всего): мужчины или женщины.
SELECT COUNT(user_id) AS 'Женщины'
FROM likes
WHERE user_id = (SELECT user_id FROM `profiles` WHERE user_id = 
	(SELECT id FROM users WHERE id = likes.user_id)
    AND gender = 'f');

SELECT COUNT(user_id) AS 'Мужчины'
FROM likes
WHERE user_id = (SELECT user_id FROM `profiles` WHERE user_id = 
	(SELECT id FROM users WHERE id = likes.user_id)
    AND gender = 'm');    
...
ON users.id = messages.from_user_id
	...
	OR messages.from_user_id IS NULL;
...
SELECT
CONCAT(u.firstname, ' ', u.lastname) AS 'Пользователь', m.from_user_id,
COUNT(m.to_user_id) AS 'Количество сообщений' 
FROM users u 
JOIN messages m 
