...
SELECT `name` AS 'Кот',
(SELECT shopname FROM shops WHERE id = cats.shops_id) AS 'Название магазина'
FROM cats
WHERE name = 'Murzik';
...
-- 4. Табличка (после слов “Последнее задание, таблица:”)
...
VALUES 
	...
	SELECT ord_datetime AS 'Время',
    (SELECT an_name FROM Analysis WHERE an_id = Orders.ord_an) AS 'Название анализа ',
    (SELECT an_price FROM Analysis WHERE an_id = Orders.ord_an) AS 'Цена анализа '
    FROM Orders
